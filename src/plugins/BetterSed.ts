...
const ourRegex = /^(?<arrows>\^*)s\/(?<find>(?:[^\\/]+?))\/(?<replace>(?:[^\\/]+?))(?:\/(?<flags>[gimsuy]*))?$/;
interface OurRegexMatch extends RegExpMatchArray {
    groups: {
        arrows: string;
        find: string;
        replace: string;
...
export default definePlugin({
    ...
    patches: [{
        find: "/^s\\/((?:.+?)[^\\\\]|.)\\/(.*)/",
        replacement: [{
            match: /(searchReplace:.{1,100},)action:function.+?}}}/,
            replace: "$1action:$self.replace"
        }, {
            match: /\/\^s\\\/.*\(\.\*\)\//,
    ...
    replace(replacement: string, { channel, isEdit }: { channel: Channel, isEdit: boolean; }) {
        ...
        const { arrows, find, replace, flags } = replacement.match(ourRegex)!.groups as OurRegexMatch["groups"];
